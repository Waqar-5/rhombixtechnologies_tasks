...
18. [Known tradeoffs & next steps](#known-tradeoffs--next-steps)
19. [Internship context](#internship-context)
...
| **Email verification is required before login** | Login is rejected with a specific `EMAIL_NOT_VERIFIED` error until the account is verified. A public resend endpoint (no login required) lets a blocked user request a fresh link right from the login page — since they can't reach the authenticated dashboard to do it otherwise |
| **Registration and password reset don't auto-log the user in** | After signing up or setting a new password, the person is redirected to `/login` rather than straight into the dashboard — account creation, password changes, and starting a session are treated as distinct, deliberate steps rather than bundled together |
...
**Verification is required before login** — see [Key product decisions](#key-product-decisions)
for the reasoning. `POST /api/auth/login` rejects an unverified account with
`403 { code: 'EMAIL_NOT_VERIFIED' }`; the frontend detects that specific code and shows an inline
"resend verification email" option right on the login page, backed by a public endpoint
(`POST /api/auth/resend-verification-public`) that doesn't require being logged in — since a
blocked user can't reach the authenticated dashboard version of that button.

**This makes correct SMTP configuration load-bearing, not just a nice-to-have.** If email sending
is broken (see [Troubleshooting](#troubleshooting)), a newly registered person has no way to
verify their account and therefore can never log in — there's no bypass. Test the full
register → receive email → verify → log in loop after any SMTP config change, and confirm the
console-log fallback is producing a usable link if you haven't configured real SMTP yet.
...
**Registered successfully but can't log in — "please verify your email"**
This is expected behavior, not a bug — see [Email verification](#email-verification). If no email
ever arrives:
1. Check the **backend terminal** — a failed SMTP send is logged there even though registration
   itself still succeeds. Look for an auth error like `Invalid login: 535-5.7.8`.
2. Confirm `SMTP_PASS` has no spaces (Gmail app passwords display with spaces for readability only)
   and `SMTP_FROM` exactly matches `SMTP_USER`.
3. Confirm you restarted the backend after editing `.env` — it's only read at startup.
4. If SMTP isn't configured at all, the link is printed directly to the backend console instead of
   emailed — copy it from there.

---

## Deployment

A split deployment — frontend and backend on different domains — needs a few things configured
that don't matter in local dev (where Vite's proxy makes everything look same-origin). This
project already handles the code-level parts; you only need to set the right env vars.

### 1. Database — MongoDB Atlas
Create a free cluster, add a database user, allow network access from anywhere (or your host's IP
range), and copy the connection string into `MONGO_URI`.

### 2. Backend — Render, Railway, or similar
- Build command: `npm install`
- Start command: `npm start`
- Environment variables: everything in `server/.env.example`, plus:
  - `NODE_ENV=production`
  - `CLIENT_URL=https://your-frontend-domain.com` (your deployed frontend's exact origin — used
    for CORS and for password-reset/email-verification redirect links)
  - `MONGO_URI` = your Atlas connection string
  - `JWT_SECRET` = a long random production secret (different from your local one)

**Uploads persistence caveat:** most free-tier hosts (Render's free plan included) use an
**ephemeral filesystem** — anything written to disk, including everything in `server/uploads/`,
is wiped on every redeploy or restart. Since this project deliberately uses local disk storage via
Multer (no Cloudinary/S3, per the original spec), that's a real limitation for a long-running
production deployment: uploaded resumes/avatars/logos won't survive a redeploy. For a portfolio
demo or short-lived deployment this is a non-issue. For anything longer-lived, either attach a
persistent disk (Render offers this on paid plans) or accept this as a known constraint of the
local-storage architecture.

### 3. Frontend — Vercel, Netlify, or similar
- Build command: `npm run build`
- Output directory: `dist`
- Environment variable: `VITE_API_URL=https://your-backend-domain.com/api` (your deployed
  backend's URL, including the `/api` suffix)

Setting `VITE_API_URL` does two things automatically:
- Points Axios at the deployed backend instead of the local dev proxy path (`client/src/api/axios.js`)
- Resolves every uploaded file URL (avatars, company logos/covers, resumes) to the backend's
  actual domain instead of the frontend's — otherwise an image tag like `/uploads/avatars/x.jpg`
  would silently try to load from the frontend's own domain and 404. This is handled centrally by
  `getFileUrl()` in `client/src/lib/utils.js`, which every image and file-download link in the app
  routes through.

### 4. Cross-origin cookies
The JWT cookie's `sameSite` setting automatically switches based on `NODE_ENV`:
`lax` in development (same-origin via the Vite proxy) and `none` in production (required for a
cookie to be sent on cross-site requests between two different domains — browsers only allow
`sameSite: none` when `secure: true`, which is enforced automatically once `NODE_ENV=production`).
No manual configuration needed — just make sure `NODE_ENV=production` is actually set on your
backend host.
...
- **Local disk storage doesn't survive a redeploy on most free hosting tiers** — see
  [Deployment](#deployment) for the full explanation. This is the direct consequence of the
  "no Cloudinary" requirement; the fix (persistent disk, or migrating to object storage) is a
  deliberate infrastructure decision, not a bug.
